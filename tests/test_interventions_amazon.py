...
def test_run_interventions_spurious_correlation_same_layer():
    ...
    test_config.probes_dir = "experiments/test_trained_amazon_probes"
    ...
    test_config.column1_vals = ["Beauty_and_Personal_Care", "Software"]
    test_config.column2_vals = [1.0, 5.0]
...
def test_run_interventions_spurious_correlation():
    ...
    test_config.dictionaries_path = "dictionary_learning/dictionaries"
    test_config.probes_dir = "experiments/trained_amazon_probes"
...
def test_run_interventions_tpp():
    ...
    for sweep_name, submodule_trainers in ae_sweep_paths.items():
        ...
        if FORCE_UPDATE_EXPECTED_RESULTS:
            with open("tests/test_data_amazon/amazon_class_accuracies_attrib_tpp.pkl", "wb") as f:
                pickle.dump(class_accuracies, f)

        with open("tests/test_data_amazon/amazon_class_accuracies_attrib_tpp.pkl", "rb") as f:
            expected_results = pickle.load(f)
...
def test_run_interventions_spurious_correlation_multiple_groupings():
    ...
    for i, column1_vals in enumerate(column1_vals_list):
        ...
        for sweep_name, submodule_trainers in ae_sweep_paths.items():
            ...
            if FORCE_UPDATE_EXPECTED_RESULTS:
                with open(
                    f"tests/test_data_amazon/amazon_class_accuracies_attrib_spurious_{column1_vals[0]}_{column1_vals[1]}.pkl",
                    "wb",
                ) as f:
                    pickle.dump(class_accuracies, f)
            ...
            with open(
                f"tests/test_data_amazon/amazon_class_accuracies_attrib_spurious_{other_professions[0]}_{other_professions[1]}.pkl",
                "rb",
            ) as f:
...
def test_run_interventions_spurious_correlation_autointerp():
    ...
    for sweep_name, submodule_trainers in ae_sweep_paths.items():
        ...
        filenames = [
            "amazon_class_accuracies_auto_interp_spurious.pkl",
            "amazon_class_accuracies_bias_shift_dir1_spurious.pkl",
            "amazon_class_accuracies_bias_shift_dir2_spurious.pkl",
        ]
        ...
        for filename in filenames:
            ...
            if FORCE_UPDATE_EXPECTED_RESULTS:
                with open(f"tests/test_data_amazon/{filename}", "wb") as f:
                    pickle.dump(class_accuracies, f)

            with open(f"tests/test_data_amazon/{filename}", "rb") as f:
                expected_results = pickle.load(f)
